...
def test_returns_version() -> None:
    ...
    assert sphinx_docsearch.__version__ == "0.3.1"
...
def test_wrong_assistant_id_key(app: Sphinx, warning: StringIO) -> None:
    ...
    app.build()
    warnings = warning.getvalue()
    assert "The docsearch_askai config value must have an assistantId key." in warnings
...
def test_correct_assistant_id_key(app: Sphinx, warning: StringIO) -> None:
    ...
    warnings = warning.getvalue()
    assert (
        "The docsearch_askai config value must have an assistantId key."
        not in warnings
    )
